...
temp$Scenario <- paste("Scenario",rep(c(1,2,4),c(nrow(g1),nrow(g2_2),nrow(g4_2))))
...
Fig2 <-  subset(temp, (SNP.Name %in% paste0("Q", c(1:10)))) %>%
  ...
  ggplot(aes(x = Scenario, y = N_Sig_QTL, col = Scenario)) +
  geom_jitter() +
    geom_hline(yintercept = 10,lty = 2)+
  ylim(c(0,12))+
  facet_grid(Nsnps2 ~ Nids2) +
  theme_bw() +
...
temp$Scenario <- paste("Scenario",rep(c(1,2,4),c(nrow(h1),nrow(h2_2),nrow(h4_2))))
...
Fig2 <-  
  ggplot(temp,aes(x = Scenario, y = H2, col = Scenario)) +
  geom_jitter() +
